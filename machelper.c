 ...
 * 
 * Mac OS Installation Helper is a command line tool to
 * create a bootable USB using Mac OS and CLOVER EFI.
 * It works only on Mac and the bootable USB only
        ...
        default:
            if (strcmp(argv[1],"usb") == 0 || strcmp(argv[1],"hdd") == 0) { // usb/hdd
                ...
                if (args == 2) { // Default procedure
                    ...
                    if (strcmp(argv[2], "-help") == 0) show_help(10);
                    else if (strcmp(argv[2], "-mountPoint") == 0) show_help(11);
                    else if (strcmp(argv[2], "-i") == 0) return mount_efi("", 1);
                    else show_help(10);
                }else if (args == 4){ // mount EFI with -mountPoint | interactive | -mountPoint help
                    if (strcmp(argv[2], "-mountPoint") == 0){ // mount EFI
                        return mount_efi(argv[3], 0);
                    }else if (strcmp(argv[2], "-i") == 0 && strcmp(argv[3], "-mountPoint") == 0){
                        show_help(11);
                    }else show_help(11);
                }else if (args == 5) {
                    if (strcmp(argv[2], "-i") == 0 && strcmp(argv[3], "-mountPoint") == 0) {
                        return mount_efi(argv[4], 1);
                    }else show_help(11);
                }else show_help(10);
            } else if (strcmp(argv[1], "extract_dsdt") == 0) { // Extract DSDT/SSDTs
                if (args == 3) { // default | help
                    if (strcmp(argv[2], "-help") == 0) show_help(4);
                    else{
                        if (argv[2][0] == '-') show_help(4);
                        else extract_dsdt(argv[2], 0);
                    }
                }else if (args == 4){ // also disassemble | extract
                    if (strcmp(argv[2], "-disassemble") == 0){ // mount EFI
                        return extract_dsdt(argv[3], 1);
                    }else show_help(4);
                }else show_help(4);
            } else if (strcmp(argv[1], "kext") == 0) { // Kext Admin
                if (args == 3) { // help | -install/-uninstall help | -rebuildCache | -fixPermission
                    // if (strcmp(argv[2], "-help") == 0) show_help(12);
                    if (strcmp(argv[2], "-install") == 0) show_help(13);
                    else if (strcmp(argv[2], "-uninstall") == 0) show_help(13);
                    ...
                    if (strcmp(argv[2], "-install") == 0){ // install kext
                        return kext_admin('i', argv[3]);
                    }else if (strcmp(argv[2], "-uninstall") == 0){ // uninstall kext
                        return kext_admin('u', argv[3]);
                    }else show_help(12);
                }else show_help(12);
            } else if (strcmp(argv[1], "recovery") == 0) { // Kext Admin
                if (args == 3) { // help | -removeKext/-replaceKernel help | -copyClover | -listUnsigned | -installOnUSB | -disableHibernation
                    // if (strcmp(argv[2], "-help") == 0) show_help(14);
                    if (strcmp(argv[2], "-removeKext") == 0) show_help(15);
                    else if (strcmp(argv[2], "-replaceKernel") == 0) show_help(15);
                    ...
                    if (strcmp(argv[2], "-removeKext") == 0){ // install kext
                        return recovery('u', argv[3]);
                    }else if (strcmp(argv[2], "-replaceKernel") == 0){ // install kext
                        return recovery('r', argv[3]);
                    }else show_help(14);
                }else show_help(14);
...
// Shows help
void show_help(int class){ // class 1 = config, 2 = ethernet, 3 = usb/hdd, 4 = extract dsdt, 5 = os, 6 = method, 7 = Raw Image Src, 8 = replace kernel, 9 = mbr patch, 10 = mount_efi, 11 = mount point, 12 = kext, 13 = install/uninstall kext, 14 = recovery, 15 = remove kext/replace kernel
    if (class == 0) { // default
        printf(
...
int run_installer(char type[]){ // type = usb|hdd
    ...
    char method;
    installation_type = (strcmp(type, "hdd") == 0) ? 'h' : 'u';
    ...
    /* === USB Formatting === */
    system("diskutil list"); // list disks to let user know the identifier of the USB
    // Unmount ESP
    sprintf(cmd, "if [ -d \"/Volumes/%s\" ]; then diskutil unmount %s; fi", _efi_name, _efi_name);
    system(cmd);
    ...
    if (installation_type == 'u' && arg_values[4][0] != '1') {
        ...
        scanf("%s", _identifier);
        if (strcmp(_identifier, "disk0") == 0) { // if IDENTIFIER is disk0
            printf("Your installation location can't be your HDD/SSD!\n");
            return -1;
        }
        ...
        if (access(cmd, F_OK) != 0) {
            printf("The partition name you entered is not available.\nInstallation Aborted.\n");
            return -1;
        }
    ...
    if (installation_type == 'u' && arg_values[4][0] == '0') {
        ...
        if (method == 'c') {
            ...
            if (access(arg_values[10], F_OK) == 0) {
                ...
                sprintf(cmd, "sudo dd if=\"%s\" of=/dev/r%s bs=1m", arg_values[10], _identifier); system(cmd);
                // Rename
                sprintf(cmd, "diskutil rename \"OS X Base System\" \"%s\"", _device_name); system(cmd);
                // Delete /Exrta folder & /kernel which is for legacy
                ...
                printf("Invalid source. Try again.\n");
                return -1;
            }
        }else{ // method = 'b'
            /* === BaseBinaries Clone Method === */
...
int extract_dsdt(char destination[], int disassemble){
    ...
    // Extract ACPI tables
    sprintf(cmd, "cd %s && ./patchmatic -extract && rm patchmatic", destination); system(cmd);
    // Renaming files
    ...
    switch (class) {
        case 'i':
            ...
            if (access(src, F_OK) == 0) {
                ...
                printf("Your kext is either inaccessible or not a valid src.\n");
            break;
        case 'u':
            // Uninstall the kext
            if (access(src, F_OK) == 0) {
        ...
        case 'r':
            ...
            system("sudo touch /S*/L*/Extensions && sudo kextcache -Boot -U /");
            break;
    }
    return 0;
}
    ...
    switch (class) {
        ...
        case 'r':
            ...
            if (access(src, F_OK)) {
                ...
                printf("Done.\n");
            }else printf("Unresolved source.\n");
            break;
        case 'd':
            // Disable Hibernation
            printf("Disabling Hibernation...\n");
            system("sudo pmset -a hibernatemode 0 && sudo rm /var/vm/sleepimage && sudo mkdir /var/vm/sleepimage");
            printf("Done.\n");
            break;
        case 'c':
            // Copy Clover from USB to HDD/SSD
            system("diskutil list");
            printf("\e[4mBe careful with this options\e[0m, as choosing wrong value(s) may cause important data to be wiped unknowingly.\n");
            ...
            system("cp -a /tmp/efi_usb/EFI/ /tmp/efi_hdd/EFI/");
            // Unmount EFIs
